...
### 新增
- “我的应用”页新增纯前端本地搜索能力
    - 支持按应用中文名、原始名称和 `appId` 模糊筛选已安装列表
    - 搜索基于前端已安装 store 本地过滤，不触发额外 IPC 或网络请求
    - 搜索为空时展示全部已安装应用，无匹配时显示独立空状态

### 修复
- 修复应用详情页“创建桌面快捷方式”未遵循 XDG 用户目录规范的问题
    - Rust 后端改为按 `XDG_DESKTOP_DIR` → `xdg-user-dir DESKTOP` → `user-dirs.dirs` → `~/Desktop` 的顺序解析桌面目录
    - 新增 `.desktop` 文件基础合法性校验，复制前确认存在合法的 `[Desktop Entry]` 组与关键字段
    - 自定义桌面目录或本地化桌面目录场景下，快捷方式现在会写入正确位置
